...
3. Para entender como gerar credenciais e tokens no ambiente de testes, consulte o [guia do sandbox](docs/itau_sandbox.md).
4. Antes de importar, siga o [guia de consentimento](docs/itau_open_finance_consent.md) para criar/autorizar o `consentId` necessário e evitar respostas 403 da API.
5. Acesse o menu **Importação → Open Finance Itaú** no app para:
...
6. Caso deseje testar manualmente, é possível informar `client_id`, `client_secret`, `consent_id`, token estático, endpoints, cabeçalhos e parâmetros extras diretamente na interface — os valores digitados ficam apenas na sessão atual.

7. Para cartões de crédito cadastrados na aba **Configurações → Contas** com dia de vencimento, continue selecionando o mês/ano de referência antes de importar para que as parcelas futuras sejam geradas corretamente.
