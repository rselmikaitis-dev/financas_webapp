...
# =====================
    ...
    df_plan = pd.read_sql_query("""
        SELECT p.ano, p.mes, p.subcategoria_id, p.valor,
               s.nome AS subcategoria, c.nome AS categoria, c.tipo
        FROM planejado p
    ...
    df_real = pd.read_sql_query("""
        SELECT strftime('%Y', t.date) AS ano,
               strftime('%m', t.date) AS mes,
               s.id AS sub_id,
               s.nome AS subcategoria,
               c.nome AS categoria,
               c.tipo,
               SUM(t.value) AS valor
        FROM transactions t
        LEFT JOIN subcategorias s ON t.subcategoria_id = s.id
        LEFT JOIN categorias c    ON s.categoria_id   = c.id
        WHERE ano=?
        GROUP BY ano, mes, sub_id
    """, conn, params=(str(ano_sel),))

    if df_plan.empty and df_real.empty:
        st.info("Nenhum dado encontrado para este ano.")
    else:
        # Ajusta meses para número
        df_plan["mes"] = df_plan["mes"].astype(int)
        df_real["mes"] = df_real["mes"].astype(int)
        ...
        df_merged = pd.merge(
            ...
            df_real,
            how="outer",
            left_on=["ano", "mes", "subcategoria_id"],
            right_on=["ano", "mes", "sub_id"],
            suffixes=("_plan", "_real")
        )

        # Corrige campos
        df_merged["categoria"] = df_merged["categoria_plan"].combine_first(df_merged["categoria_real"])
        df_merged["subcategoria"] = df_merged["subcategoria_plan"].combine_first(df_merged["subcategoria_real"])
        df_merged["tipo"] = df_merged["tipo_plan"].combine_first(df_merged["tipo_real"])

        df_merged["planejado"] = df_merged["valor_plan"].fillna(0)
        df_merged["realizado"] = df_merged["valor_real"].fillna(0).abs()  # despesas ficam positivas
        df_merged["diferenca"] = df_merged["realizado"] - df_merged["planejado"]
        df_merged["desvio %"] = df_merged.apply(
            lambda r: (r["diferenca"] / r["planejado"] * 100) if r["planejado"] != 0 else 0,
            axis=1
        )
        ...
        df_merged["Mês"] = df_merged["mes"].map(meses_nomes)

        # Reordena colunas
        df_final = df_merged[[
            "categoria", "subcategoria", "Mês", "planejado", "realizado", "diferenca", "desvio %"
        ]].sort_values(["categoria", "subcategoria", "Mês"])
        ...
        df_fmt["Planejado"] = df_fmt["Planejado"].map(brl_fmt)
        df_fmt["Realizado"] = df_fmt["Realizado"].map(brl_fmt)
        df_fmt["Diferença"] = df_fmt["Diferença"].map(brl_fmt)
        df_fmt["Desvio (%)"] = df_fmt["Desvio (%)"].map(lambda x: f"{x:.1f}%" if x != 0 else "-")
